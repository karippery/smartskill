    ...
    def handle(self, *args, **kwargs):
            ...
            for user in User.objects.all():
                for skill in Skill.objects.all():
                    user_skills.append(UserSkill(user_id=user, skill_id=skill, level=fake.random_int(min=1, max=5)))
            UserSkill.objects.bulk_create(user_skills)

            self.stdout.write(self.style.SUCCESS('Successfully created fake data for SkillCategory, Skill, and UserSkill.'))